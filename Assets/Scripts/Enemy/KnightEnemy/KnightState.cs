...
public class KnightState : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (precas.Length > 0 && _patrol == false)
        {
    ...
    {
        _knight.SetMover(new PointByPointMover(_knight.transform, _patrolPoints.Select(point => point.position), _speed, _animationObject, _knight));
    }

    private void StartPersecution()
    {
        _knight.SetMover(new MoveToTargetPatern(_knight.transform, _fov.visibleTargets[0], _speed, _knight, _fov,_animationObject));
    }

    private void StartIdle()
    {
        _knight.SetMover(new NoMove(_speed, _knight));
    }
}
